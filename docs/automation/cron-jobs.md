...
注意：你也可以在主会话任务上设置 `model`，但它会改变共享的主会话模型。建议仅在隔离任务上使用模型覆盖，以避免意外的上下文切换。
...
隔离任务（`agentTurn`）可以设置 `lightContext: true` 来使用轻量引导上下文运行。

- 用于不需要工作区引导文件注入的计划任务。
- 实际上，嵌入式运行时会以 `bootstrapContextMode: "lightweight"` 运行，这会故意保持 cron 引导上下文为空。
- CLI 等效命令：`openclaw cron add --light-context ...` 和 `openclaw cron edit --light-context`。

### 投递（频道 + 目标）

隔离任务可以通过顶级 `delivery` 配置将输出投递到频道：

- `delivery.mode`：`announce`（频道投递）、`webhook`（HTTP POST）或 `none`。
- `delivery.channel`：`whatsapp` / `telegram` / `discord` / `slack` / `mattermost`（插件）/ `signal` / `imessage` / `last`。
...
- Slack/Discord/Mattermost（插件）目标应使用显式前缀（例如 `channel:<id>`、`user:<id>`）以避免歧义。
  Mattermost 裸 26 字符 ID 按 **用户优先** 解析（如果用户存在则发私信，否则视为频道）——使用 `user:<id>` 或 `channel:<id>` 确保确定性路由。
- Telegram 主题应使用 `:topic:` 格式（见下文）。
