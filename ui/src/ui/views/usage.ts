...
export function renderUsage(props: UsageProps) {
  ...
  const datePresets = [
    ...
    { label: t("usage.sevenDays"), days: 7 },
    { label: t("usage.thirtyDays"), days: 30 },
  ];
      ...
      >
        <summary>
          ...
          ${
            selectedCount > 0
              ? html`
                  <span class="usage-filter-badge">${selectedCount}</span>
                `
              : html`
            ...
            displayTotals
              ? html`
                <span class="usage-metric-badge">
                  <strong>${formatTokens(displayTotals.totalTokens)}</strong> ${t("usage.tokensLabel")}
                </span>
                <span class="usage-metric-badge">
                  <strong>${formatCost(displayTotals.totalCost)}</strong> ${t("usage.costLabel")}
                </span>
                <span class="usage-metric-badge">
      ...
      <div style="margin-top: 12px;">
          ...
          <div class="usage-query-actions">
            ...
            <span class="usage-query-hint">
              ${
                hasQuery
                  ? `${filteredSessions.length} of ${totalSessions} ${t("usage.sessionsMatch")}`
                  : `${totalSessions} ${t("usage.sessionsInRange")}`
              }
            </span>
          </div>
        </div>
