...
function renderFallbackIndicator(status: FallbackIndicatorStatus | null | undefined) {
  ...
  const details = [
    `${t("chat.selected")} ${status.selected}`,
    phase === "cleared" ? `${t("chat.active")} ${status.selected}` : `${t("chat.active")} ${status.active}`,
    phase === "cleared" && status.previous ? `${t("chat.previousFallback")} ${status.previous}` : null,
    status.reason ? `${t("chat.reason")} ${status.reason}` : null,
    status.attempts.length > 0 ? `${t("chat.attempts")} ${status.attempts.slice(0, 3).join(" | ")}` : null,
  ]
    .filter(Boolean)
    .join(" \u2022 ");
  const message =
    phase === "cleared"
      ...
      : `${t("chat.fallbackActive")} ${status.active}`;
  const className =
    phase === "cleared"
